...
$reserved=$_GET['res_id'];
if (isset($_GET['category_id'])){
    mysqli_query($conn,"UPDATE `reservation` SET `status` = 'done' WHERE `reservation`.`reserve_id` = '$reserved'");
}
?>
								...
								<form id="filter">
									<div class="row">
										...
										<div class="col-md-2">
											...
											<button class="btn btn-block btn-outline-primary">Filter</button>
										</div>
									</div>
								</form>
							</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
											<button  class="btn btn-sm btn-outline-primary check_in" type="button" data-id="<?php echo $row['id'] ?>"  >Check-in</button>
									</td>
								</tr>
							<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.check_in').click(function(){
		uni_modal("Check In","manage_check_in_res.php?rid="+$(this).attr("data-id")+"&res_id="+<?php echo $reserved?>)
	})
