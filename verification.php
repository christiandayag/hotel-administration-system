    ...
    if(isset($_POST["verify"])){
        ...
        if($otp != $otp_code){
           ...
           <?php
            ...
            mysqli_query($conn, "UPDATE account SET status = 1 WHERE email = '$email'");
            $select = mysqli_query($conn, "Select * from account where  email = '$email'");
            if (mysqli_num_rows($select) > 0){
                $row = mysqli_fetch_assoc($select);
                mysqli_query($conn,"INSERT INTO `users` ( `name`, `username`, `password`, `type`) VALUES ('".$row['name']."','".$row['username']."', '".$row['password']."', '2')");
            }
            ?>
             <script>
